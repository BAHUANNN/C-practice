...
const int FRACTALS_LEFT_X = 500;
const int FRACTALS_LEFT_Y = 360;
...
void demoFractals(GWindow & w) {
    ...
    w.clear();
    int X = FRACTALS_LEFT_X;
    int Y = FRACTALS_LEFT_Y;
    for(int i = 1; i <= 4; i++) {
        cout << "Here is serpinskii order " << i << endl;
        serpinskii(w, 0, X, X/2, 0, 0, Y, FRACTALS_SIZE, i);
        getLine("Press enter to contine: ");
        w.clear();
    }

    cout << "Here is serpinskii order 7" << endl;
    serpinskii(w, 0, X, X/2, 0, 0, Y, FRACTALS_SIZE, 7);
    waitToReturnToMenu();
}
